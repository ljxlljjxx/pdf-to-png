...
if __name__ == "__main__":
    ...
    if input('是否合并?(回答yes或no)') == 'yes':
        images = []
        i = 0
        while (Path(imagePath) / f'images_{i}.png').exists():
            images += [f'{imagePath}/images_{i}.png']
            i += 1
        image_path = input('合并后的png地址:')

        Gather.concatenate_vertical(images, image_path)
